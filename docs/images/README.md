# Images

Diagrams and screenshots referenced from documentation.

## Expected files

| File | Referenced by | Status |
|---|---|---|
| `overview.png` | `README.md:5` | Committed |
| `ai_launchpad_double_diamond.svg` | [`../ai-assistant-problem-solution.md`](../ai-assistant-problem-solution.md) | Not yet produced |
| `ai_launchpad_moscow.svg` | [`../ai-assistant-problem-solution.md`](../ai-assistant-problem-solution.md) | Not yet produced |

## Conventions

Prefer SVG for diagrams — it stays legible at any zoom and diffs meaningfully. Use PNG for screenshots.

Name files after what they show, not where they appear, so that moving a reference does not strand the filename.