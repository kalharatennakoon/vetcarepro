...
Every chat call runs on one shared context window, `OLLAMA_NUM_CTX` (8192), set in `ml/scripts/rag/ollama_client.py`. This is not a tuning knob to leave alone: Ollama's own 4096 default is smaller than several of this app's prompts (a pet-owner question measures ~3600 tokens before the answer starts), and it handles an overrun by silently dropping the **front** of the context — the system prompt first, then the retrieved records — and carrying on, reporting `done_reason: "stop"` exactly as it would for a clean finish. The visible symptom is an answer that stops mid-thought. Two consequences worth keeping: the window must stay one value across every call, or Ollama evicts and reloads the whole 5.6GB model between the two calls of a single explain/summarize turn (it keys its loaded instance partly on `num_ctx`); and overrun is detected after the fact from the reported token counts, so the ML service logs `[ollama] ... hit the N-token context window` whenever it happens — a recurring warning there means raise `OLLAMA_NUM_CTX`, gradually, watching `ollama ps` and system memory (16384 once OOM-crashed Ollama outright on this hardware). On a thinking-mode call the client also retries once without thinking, since the reasoning pass is what consumes the room the answer needed.
...
3. **`pet_health_intent.py`** — an individual pet's disease-recurrence/cancer risk, and clinic-wide pandemic risk, computed live by `PetHealthPredictor` — never through RAG retrieval, same "live model, not a text sample" reasoning as the pre-pipeline gates above. Restricted to `PET_HEALTH_ROLES` (admin and veterinarian, same set as `CLINICAL_STAFF_ROLES` — vets need these predictions for actual clinical decisions, not just admin oversight); the module gates internally, returning `None` for guest/pet_owner (ordinary general-knowledge territory for them) but an explicit role-denial for receptionist. The admin-only "show reasoning" trace is a separate, narrower restriction — vets get the prediction, not the reasoning.
